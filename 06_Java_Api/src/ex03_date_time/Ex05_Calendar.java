...
public class Ex05_Calendar {
	...
	public static void main(String[] args) {
		...
		// 년 월 일 요일 가져오기
		int year = now.get(Calendar.YEAR); 			// now.get(1);과 동일한 동작
		int month = now.get(Calendar.MONTH) + 1;	// 월 (0 ~ 11) : 주의 필요
		int day = now.get(Calendar.DAY_OF_MONTH);	
		int weekNo = now.get(Calendar.DAY_OF_WEEK);	// 일(1)~토(7)
		
		System.out.println(year);
		System.out.println(month);
		System.out.println(day);
		
		String[] wkn = {" ","일","월","화","수","목","금","토"};
		
		System.out.println(wkn[weekNo]);
