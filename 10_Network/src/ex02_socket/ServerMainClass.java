	...
	public static void main(String[] args) {
		...
		try {
			...
			while(true) {
				...
				// 클라이언트에게 인사 메세지 보내기
				DataOutputStream out = new DataOutputStream(clientSocket.getOutputStream()); // 접속한 클라이언트로 데이터를 보내는 스트림
				out.writeUTF("어서오세요. 환영합니다."); //writeUTF 메소드를 이용하면 바이트스트림으로도 깨짐없이 데이터를 보낼 수 있다.
				...
				System.out.println("[클라이언트] " + sb.toString());

				// 스트림 종료
				out.close();
				in.close();

				// 클라이언트 접속 종료 (생략 가능 - 클라이언트클래스에서 종료해주기 때문)
				clientSocket.close();
